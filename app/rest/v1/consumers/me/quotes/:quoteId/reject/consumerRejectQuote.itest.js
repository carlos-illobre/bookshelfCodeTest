...
const rejectQuoteUrl = `${process.env.WEB_DOMAIN}/rest/v1/consumers/me/quotes/{quoteId}/reject`
...
describe('consumerAcceptQuote', () => {
  ...
  it('reject quote', () => {
    ...
    .then(([contractorId, consumerId, quoteId, consumerJwt]) => Promise.all([
      ...
      consumerId,
      quoteId,
      chakram.put(rejectQuoteUrl.replace('{quoteId}', quoteId), {}, {
    ...
    .then(([contractorId, consumerId, quote]) => {
      ...
      expect(quote.get('state')).to.equals(quoteStates.rejected)
    })
  })

  it('returns 401 if not jwt', () => chakram.put(rejectQuoteUrl)
    .then(response => {
      expect(response).to.have.status(401)
    })
  )
  
  it('returns 401 if bad jwt', () => chakram.put(rejectQuoteUrl, {}, {
    headers: { jwt: 'bad_jwt' } })
    .then(response => {
      expect(response).to.have.status(401)
    })
